...
Deno.serve(async (req) => {
  try {
    ...
    if (!customer || !customer.customer_id) {
      customerId = await createNewStripeCustomer(user, mode);
    } else {
...
async function createNewStripeCustomer(user: any, mode: string): Promise<string> {
  ...
  // Use upsert to handle cases where stale customer ID might exist
  ...
  if (mode === 'subscription') {
    ...
    if (upsertSubscriptionError) {
      console.error('Failed to save subscription in the database', upsertSubscriptionError);
      ...
      throw new Error('Unable to save the subscription in the database');
    }
  }

  console.log(`Successfully set up new customer ${newCustomer.id} with subscription record`);
